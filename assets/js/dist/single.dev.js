...
// 定数
var single_text_id = "single-text"; // 変数

var single_text = ""; // 投稿のテキスト

var single_imgs = {}; // 投稿の画像の文字列(連想配列)

var single_text_elem; // 投稿のテキストの要素

var single_img_elem; // 投稿の画像の要素

var single_text_boxes_top = {}; // 投稿の各テキストのトップ

var single_text_boxes_bottom = {}; // 関数
// 投稿を整理する

function organizeContents(contents) {
  // 重複削除
  var new_contents = Array.from(new Set(contents)); // \n\nを削除
...
function divideTextImg(contents) {
  ...
  for (var iter = 0; iter < contents.length; iter++) {
    // divタグ生成
    single_text += '<div id="' + single_text_id + id_num + '" class="text-box">';

    while (!existImgTag(contents[iter])) {
      // imgタグがない場合
    ...
    single_text += '</div>\n';
    var html_doc = parser.parseFromString(contents[iter], "text/html");
    var img = html_doc.querySelector("img");
...
function displayText(text) {
  ...
  var text_elem = document.getElementById(single_text_id);
...
function setTextBoxesRect() {
  var text_boxes_elem = single_text_elem.getElementsByClassName("text-box");

  for (var i = 0; i < text_boxes_elem.length; i++) {
    var text_boxes_rect = text_boxes_elem.item(i).getBoundingClientRect();
  ...
  single_text_boxes_top[single_text_id + "0"] = 0; // return [tops, bottoms];
...
function changeImg() {
  // スクロールされたときのトップからの距離
  var scroll_top = $(window).scrollTop(); // for文でimgの配列文探索する。switchでは、ダメ
  // topとbottom間で画像の切り替えを行う

  for (var i = 0; i < Object.keys(single_imgs).length; i++) {
...
window.addEventListener("load", function () {
  ...
  divideTextImg(organizeContents(contents)); // テキスト
  // テキスト表示
  // console.log(single_text);

  displayText(single_text); // テキストの要素を取得(表示されたから)

  setElementSingleText(); // 各テキストの要素

  setTextBoxesRect(); // 画像

  setElementImg(); // onLoadSingleImg(single_imgs[single_text_id+"0"]);
